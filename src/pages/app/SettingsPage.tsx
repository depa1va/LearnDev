...
import { getAvatarInitials, getPrivateProfile, updateUserProfile, validateProfileFields } from '../../services/profileService';
...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState<ProfileFormValues>({ displayName: '', bio: '' });
  ...
  function updateField(field: keyof ProfileFormValues, value: string): void {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      setForm({ displayName: updatedProfile.displayName ?? '', bio: updatedProfile.bio ?? '' });
      setStatus('saved');
  ...
  return (
    <PageFrame>
      ...
      {isLoading ? <div className="rounded-3xl bg-white px-6 py-12 text-center text-sm text-ink/55 shadow-sm">Carregando suas configurações...</div> : loadError ? <EmptyState icon={Settings2} title="Não foi possível carregar as configurações." description={loadError} /> : profile && <form onSubmit={handleSubmit} className="max-w-3xl rounded-3xl border border-ink/5 bg-white p-6 shadow-soft sm:p-8">
        <div className="mb-8 flex items-center gap-4 rounded-2xl bg-mist p-5">
          <Avatar initials={getAvatarInitials(profile.displayName, profile.username)} photoURL={profile.photoURL} size="lg" />
          <div>
            <h2 className="font-heading text-lg font-semibold text-ink">Foto do perfil</h2>
            <p className="mt-1 text-sm leading-relaxed text-ink/60">Enquanto o envio de imagens não for ativado no Storage do projeto, seu avatar usa as iniciais do seu nome.</p>
          </div>
        </div>
        ...
        <div className="mt-7 flex justify-end"><button type="submit" disabled={status === 'saving'} className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-white shadow-soft hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"><Save className="h-4 w-4" />{status === 'saving' ? 'Salvando...' : 'Salvar alterações'}</button></div>
      </form>}
    </PageFrame>
  );
}
